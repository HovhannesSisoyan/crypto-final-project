...
void string_to_matrix(std::string s, std::string matr[4][4]) {
  for (int i = 0; i < 4; i++) {
    for(int j = 0; j < 4; j++) {
      matr[j][i] = std::string() + s.at(i * 8 + j*2) + s.at(i * 8 + j*2 + 1);
    }
  }
}
...
int main() {
  ...
  string_to_matrix(input, input_state);
  ...
  input_key = to_lower_case(input_key);
  string_to_matrix(input_key, initial_key);
